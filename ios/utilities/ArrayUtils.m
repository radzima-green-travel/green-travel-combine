...
NSArray* shuffledArray(NSArray *items) {
  ...
  return [shuffledItems subarrayWithRange:NSMakeRange(0, MIN(5, [shuffledItems count]))];
}

NSArray* slice(NSArray *items) {
  NSMutableArray *itemsToSlice = [NSMutableArray arrayWithArray:items];
  return [itemsToSlice subarrayWithRange:NSMakeRange(0, MIN(5, [itemsToSlice count]))];
}
